function spt_linear_points_fit(data)

[nFramesPerTrack,~] = cellfun(@size,data{1}.tracks,'UniformOutput',false);
maxPoints = max(vertcat(nFramesPerTrack{:}));

input_values = inputdlg({['Number of time points to fit (0 - ' num2str(maxPoints) '):']},'',1,{num2str(3)});
if isempty(input_values)==1
    return
else
    nPoints=str2double(input_values{1});    
    for i=1:length(data)
        [data{i}.a,data{i}.b,data{i}.r2] = msd_fit_linear_msd_inside(data{i}.msd,nPoints);         
    end    
    msd_fit_linear_msd_plot(data)
end
end

function [a,b,r2] = msd_fit_linear_msd_inside(data,nPoints)
ft = fittype('a*x + b');
f = waitbar(0,'Fitting Linear Function');
for i=1:length(data)
    waitbar(i/length(data),f,['Fitting Linear Function - track ' num2str(i) ' of ' num2str(length(data))])
    if size(data{i},1)>=nPoints
        data_to_fit = data{i}(1:nPoints,:);
    else
        data_to_fit = data{i};
    end        
    ...
    function save_data(~,~,~)
        ...
        if path~=0
            for i = 1:length(data)
                ...
                data_to_save(:,2) = cell2mat(data{i}.b);
                data_to_save(:,3) = data_to_save(:,1)/4;
                data_to_save(:,4) = cell2mat(data{i}.r2);
                table_data = array2table(data_to_save,'VariableNames',{'a','b','D','r2'});
                f = waitbar(0,'Saving...');
...
title({['r² = ',num2str(r2),', a = ',num2str(a),', b = ',num2str(b),', ax+b'],['Diffusion Coefficient (a/4) = ',num2str(diffusion_coefficient), ' µm²/s'],['File Name = ',regexprep(name,'_',' ')],['Track Number = ',num2str(slider_two_value),'/',num2str(length(input_data))]},'interpreter','latex','FontName','TimesNewRoman','fontsize',14)
