...
for i = 1:numel(RefIndex)
    ...
    for j = size(ClustersRef{i},1)
        ...
        for k = 1:size(IdxClusters{i}{j},1)
            ...
            ColocCoords = ClustersColoc{i}{IdxClusters{i}{j}(k)}(:,1:2); % Extract the coordinates for the potentially interesting cluster
    ...
    ColocalizedClusters{i} = {ClustersColoc{i}{ColocClusterIds}}';
    NotColocClusterIds = setdiff(1:size(ClustersColoc{i},1),ColocClusterIds)';
    NotColocalizedClusters{i} = {ClustersColoc{i}{NotColocClusterIds}}';
    % Clear up the work space
...
% Clear up the work space
clear i input_values Continue FilteredColoc IdxClusters ClustersColoc ClustersRef
