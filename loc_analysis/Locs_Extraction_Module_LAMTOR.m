function Locs_Extraction_Module_LAMTOR()
...
figure();
set(gcf,'name','Localization Extraction LAMTOR Module','NumberTitle','off','color','k','units','normalized','position',[0.25 0.2 0.5 0.6],'menubar','none','toolbar','figure');
...
data_colocalization = [];
    ...
    function colocalization_callback(~,~,~)
        ...
        if isempty(input_values)
            ...
            Expansion_LAMTOR = str2double(input_values{1});
            Expansion = 2-str2double(input_values{1});
            BgExpansion = str2double(input_values{2});
            ColocThreshold = str2double(input_values{3});
            Stats = str2double(input_values{4});

            if Stats == 1
                [file,path] = uiputfile('NewLysoMask_LAMTOR_Extraction.xlsx','Please specify a name to save the statistics as'); % Extract the name of the file given.
            ...
            if ~isempty(data_reference) && ~isempty(data_colocalization)
                ...
                if length(data_reference)==length(data_colocalization)
                    ...
                    data_ref_coloc = cell(length(data_reference),1);
                    ...
                    for i = 1:length(data_reference)
                        ...
                        [data_inside{i},data_insidedonut{i},data_insideBgdonut{i},data_notinside{i},data_insideHigher{i},data_insideLower{i},data_ref_none{i},data_ref_coloc{i},TableCell{i}] = find_extract_locs(data_reference{i},data_colocalization{i},Expansion_LAMTOR,Expansion,BgExpansion,ColocThreshold,Stats,counter); % See inner function for more explanation.
                    ...
                    if Stats == 1
                        ...
                        Sheetname = cell(numel(data_reference),1);
                        for i = 1:numel(data_reference)
                            ...
                            if length(data_reference{i}.name) > 31
                                Sheetname{i} = data_reference{i}.name(1:31);
                            else
                                Sheetname{i} = data_reference{i}.name;
                            end
                        ...
                        for i = 1:numel(data_reference)
                            writetable(Table_toWrite{i},name,'sheet',Sheetname{i}); % Write the table to the Excel file.
                        end
                    end
...
function [data_inside,data_insidedonut,data_insideBgdonut,data_notinside,data_insideHigher,data_insideLower,data_ref_none,data_ref_coloc,TableCell] = find_extract_locs(data_reference,data_colocalization,Expansion_LAMTOR,Expansion,BgExpansion,ColocThreshold,Stats,counter_waitbar)
...
if Expansion_LAMTOR ~= 0
    PolygonsRefsLAMTOR = cellfun(@(x) polybuffer(x,Expansion_LAMTOR),PolygonsRefs,'UniformOutput',false);
else
    PolygonsRefsLAMTOR = PolygonsRefs;
end

IsInsideCluster = cellfun(@(x) inpolygon(DataColoc(:,1),DataColoc(:,2),x.Vertices(:,1),x.Vertices(:,2)),PolygonsRefsLAMTOR,'UniformOutput',false);
...
% 
%     OverlappedDonuts = triu(overlaps(vertcat(PolygonsRefsLAMTOR{:})),1); % The 1 is because we do not care about the diagonal itself.
...
%     if ~isempty(Chain)
%         for i = 1:numel(Chain)
%             PolygonsRefsLAMTOR{Chain{i}(1)} = union(vertcat(PolygonsRefsLAMTOR{Chain{i}}));
%             ClustersRef{Chain{i}(1)} = vertcat(ClustersRef{Chain{i}});
%         end
%         Chain = cellfun(@(x) x(2:end),Chain,'UniformOutput',false);
%         try
%             allRemoved = horzcat(Chain{:});
%         catch
%             allRemoved = vertcat(Chain{:});
%         end
%         ClustersRef(allRemoved) = [];
%         PolygonsRefsLAMTOR(allRemoved) = [];
%     end
%     PolygonsRefs = PolygonsRefsLAMTOR;
%     BoundaryCoords = cellfun(@(x) boundary(x),PolygonsRefs,'UniformOutput',false);
% 
% end
% 
...
if Stats == 1
    if BgExpansion == 0
        TableCell = cell(numel(PolygonsRefs)+1,12);
    else
        TableCell = cell(numel(PolygonsRefs)+1,17);
        TableCell(:,13:17) = {NaN};
    end
    TableCell{1,1} = data_reference.name;
    TableCell(1,2:end) = {NaN};
    TableCell(2:end,1) = {NaN};
    TableCell(2:end,2) = num2cell((1:numel(PolygonsRefs))');
    
end

PolygonsRefsExpanded = cellfun(@(x) polybuffer(x,Expansion+Expansion_LAMTOR),PolygonsRefs,'UniformOutput',false);
if BgExpansion ~= 0
...
if Stats == 1
    ...
    PolygonRefArea = cellfun(@(x) area(x),PolygonsRefsLAMTOR);
    PolygonRefperim = cellfun(@(x) perimeter(x),PolygonsRefsLAMTOR);
...
if ~isempty(DataInsideCluster)
    ...
    data_inside.name = [data_colocalization.name,'_InsideCluster_Refplus' num2str(Expansion_LAMTOR) 'PixelRing_' num2str(Expansion_LAMTOR+Expansion) 'PixelExpansion_' num2str(BgExpansion) 'BgPixelExpansion'];
    if isfield(data_reference,'coordinates')
...
if ColocThreshold > 0
    ...
    if ~isempty(DataInsideCluster_Higher)
        ...
        data_insideHigher.name = [data_colocalization.name,'_InsideClusterHigher_Refplus' num2str(Expansion_LAMTOR) 'PixelRing_' num2str(Expansion_LAMTOR+Expansion) 'PixelExpansion_' num2str(BgExpansion) 'BgPixelExpansion'];
        if isfield(data_reference,'coordinates')
    ...
    if ~isempty(DataInsideCluster_Lower)
        ...
        data_insideLower.name = [data_colocalization.name,'_InsideClusterLower_Refplus' num2str(Expansion_LAMTOR) 'PixelRing_' num2str(Expansion_LAMTOR+Expansion) 'PixelExpansion_' num2str(BgExpansion) 'BgPixelExpansion'];
        if isfield(data_reference,'coordinates')
...
function input_values = InputDialog()
    ...
    uicontrol('Style','text','Units','Normalized','Position',[.05 .85 .6 .1],'String','LAMTOR Ring Expansion (pixels): ','FontSize',10,'HorizontalAlignment','right');
    Expansion = uicontrol('Style','Edit','Units','Normalized','Position',[.72 .85 .12 .1],'String','0.5','FontSize',10);
